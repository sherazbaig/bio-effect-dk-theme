/**
 * Plugin: Basis icon shortcode plugin
 * -----------------------------------------------------------------------------
 * Replaces 'data:~icon...' shortcodes in assets with base64 versions.
 * - Functionality duplicated in Storybook Icon Shortcode Plugin.
 *
 */
 ...
 * Set variables.
 */
const pluginName = 'BasisIconShortcodePlugin'

/**
 * Tap into hooks and add functionality.
 */
module.exports = class BasisIconShortcodePlugin {
  apply(compiler) {
  ...
  replaceIconShortcodes(compilation, assets) {
    return new Promise(async(resolve, reject) => {
      try {
        ...
        /**
         * Filter out non-CSS files from assets.
         */
        ...
        debug('Icons', 'Replaced icon shortcodes', 'bgMagenta')
        resolve()

      } catch (error) {
        debug('Icons 1', error, 'bgRed')
        reject(error)
      }
    })
  }
